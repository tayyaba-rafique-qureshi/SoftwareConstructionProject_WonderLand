...
async function nextStep() {
    ...
    // LOGIC: Moving from Age (Step 0) -> Interest (Step 1)
    if (step === 0) {
        const age = answers.ageGroup;
        
        // 1. If Toddler (0-2), SKIP fetching categories and finish
        if (age === "0-2") {
            answers.interest = ""; // No specific interest
            submitQuizToBackend();
            return;
        }

        // 2. Fetch valid categories for this age
        ...
        try {
            ...
            // Map strings to options objects
            ...
            nextBtn.disabled = false; // Re-enable if we didn't move
        }
        return;
    }
...
function restartQuiz() {
    ...
    questions[1].options = []; // Reset dynamic options
